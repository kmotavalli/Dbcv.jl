...
for (testname, *custom_params) in test_parameters:
	...
	match testname:
		case "low_rank_matrix":
			subprocess.run(["python3", os.path.join(test_scripts_dir, "generalized_tester_prim.py"), "--testname", testname, "--numsamples", str(params["samples"]),
				"--eps", str(params["eps"]), "--n_features", str(params["n_features"])])
		case "biclusters" | "checkerboard":
			subprocess.run(["python3", os.path.join(test_scripts_dir, "generalized_tester_prim.py"), "--testname", testname, "--shape", str(params["shape"][0]), str(params["shape"][1]), "--n_clusters", str(params["n_clusters"]), "--numsamples", str(params["samples"]),
		...
		case "medical":
			...
			for csv_file in csv_files:
				subprocess.run(["python3", os.path.join(test_scripts_dir, "generalized_tester_prim.py"), "--testname", os.path.join(csv_dir, csv_file), "--eps", str(params["eps"]) ])
